...
public class AntTurnLeftFunction extends Node {
	...
	/**
	 * Constructs an <tt>AntTurnLeftFunction</tt> with one <tt>null</tt> child
	 */
	...
	/**
	 * Constructs an <tt>AntTurnLeftFunction</tt> with one child node. The given
	 * child must have a return-type of <tt>Ant</tt>
	 * 
	 * @param child this node's only child.
	 */
	public AntTurnLeftFunction(Node child) {
		super(child);
	}

	/**
	 * Constructs an <tt>AntTurnLeftFunction</tt> with no child nodes, but
	 * the given ant which will be held internally. This makes the function a
	 * terminal node with arity 0. Note that this differs from the
	 ...
	 *        <tt>null</tt>.
	 */
	public AntTurnLeftFunction(Ant ant) {
		super();
	...
	/**
	 * Returns the identifier of this function which is <tt>TURN-LEFT</tt>
	 */
	@Override
