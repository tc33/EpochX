...
/**
 * Unit tests for {@link org.epochx.epox.math.Max2Function}
 */
public class Max2FunctionTest extends NodeTestCase {

	private Max2Function max;
	...
	public void setUp() {
		...
		max = new Max2Function(child1, child2);

		super.setUp();
	}
	
	/**
	 * Tests that {@link org.epochx.epox.math.Max2Function#evaluate()} 
	...
	public void testEvaluateDouble() {
		...
		assertEquals("MAX of 0.0 and 0.0 should be 0.0", 0.0, result);
		
		child1.setEvaluate(2.1);
		child2.setEvaluate(3.1);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Double.class, result.getClass());
		assertEquals("MAX of 2.1 and 3.1 should be 3.1", 3.1, result);
	
		child1.setEvaluate(-3.1);
		child2.setEvaluate(2.1);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Double.class, result.getClass());
		assertEquals("MAX of -3.1 and 2.1 should be 2.1", 2.1, result);
	}
	...
	public void testEvaluateInteger() {
		...
		assertEquals("MAX of 0 and 0 should be 0", 0, result);
		
		child1.setEvaluate(2);
		child2.setEvaluate(3);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Integer.class, result.getClass());
		assertEquals("MAX of 2 and 3 should be 3", 3, result);
	
		child1.setEvaluate(-3);
		child2.setEvaluate(2);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Integer.class, result.getClass());
		assertEquals("MAX of -3 and 2 should be 2", 2, result);
	}
	...
	public void testEvaluateLong() {
		...
		assertEquals("MAX of 0L and 0L should be 0L", 0L, result);
		
		child1.setEvaluate(2L);
		child2.setEvaluate(3L);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Long.class, result.getClass());
		assertEquals("MAX of 2L and 3L should be 3L", 3L, result);
	
		child1.setEvaluate(-3L);
		child2.setEvaluate(2L);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Long.class, result.getClass());
		assertEquals("MAX of -3L and 2L should be 2L", 2L, result);
	}
	...
	public void testEvaluateFloat() {
		...
		assertEquals("MAX of 0.0f and 0.0f should be 0.0f", 0.0f, result);
		
		child1.setEvaluate(2.1f);
		child2.setEvaluate(3.1f);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Float.class, result.getClass());
		assertEquals("MAX of 2.1f and 3.1f should be 3.1f", 3.1f, result);
	
		child1.setEvaluate(-3.1f);
		child2.setEvaluate(2.1f);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Float.class, result.getClass());
		assertEquals("MAX of -3.1f and 2.1f should be 2.1f", 2.1f, result);
	}
	...
	public void testEvaluateMixed() {
		...
		Object result = max.evaluate();
		assertSame("Return type should be widest of input types", Double.class, result.getClass());
		assertEquals("MAX of 1 and 2.4 should be 2.4", 2.4, max.evaluate());
		
		child1.setEvaluate(3.1f);
		child2.setEvaluate(2.1d);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Double.class, result.getClass());
		assertEquals("MAX of 3.1f and 2.1d should be 3.1d", 3.1d, (Double) result, 1);
	
		child1.setEvaluate(6L);
		child2.setEvaluate(2);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Long.class, result.getClass());
		assertEquals("MAX of 6L and 2 should be 6L", 6L, result);
		
		child1.setEvaluate(2.1f);
		child2.setEvaluate(3);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Float.class, result.getClass());
		assertEquals("MAX of 2.1f and 3 should be 3.0f", 3.0f, result);
		
		child1.setEvaluate(-2.1f);
		child2.setEvaluate(2L);
		result = max.evaluate();
		assertSame("Return type should be widest of input types", Float.class, result.getClass());
		assertEquals("MAX of -2.1f and 2L should be 2.0f", 2.0f, result);
	}
	...
	public void testEpoxParser() {
		...
		try {
			...
			Node n = parser.parse("MAX(X, X)");
			assertSame("Parsing did not return an instance of the correct node", Max2Function.class, n.getClass());
