...
public class SineFunctionTest extends NodeTestCase {
	...
	/**
	 * Tests that {@link org.epochx.epox.trig.SineFunction#dataType(Class...)}
	...
	public void testGetReturnTypeSin() {
		...
		for (Class<?> type: inputTypes) {
			returnType = sin.dataType(type);
			assertSame("unexpected return type", Double.class, returnType);
		}
		
		returnType = sin.dataType(Boolean.class);
		assertNull("non-numeric type for child should be invalid", returnType);
		
		returnType = sin.dataType(Integer.class, Integer.class);
	...
	public void testEpoxParser() {
		...
		try {
			...
			Node n = parser.parse("SIN(X)");
			assertSame("Parsing did not return an instance of the correct node", SineFunction.class, n.getClass());
