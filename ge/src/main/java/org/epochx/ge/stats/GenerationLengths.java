...
public class GenerationLengths extends AbstractStat<EndGeneration> {
	...
	/**
	 * Constructs a <code>GenerationChromosomeLengths</code> stat and registers
	 * its dependencies
	 */
	...
	public void refresh(EndGeneration event) {
		...
		for (Individual individual: population) {
			if (individual instanceof GEIndividual) {
				lengths[index++] = ((GEIndividual) individual).getChromosome().length();
			}
		}
	}

	/**
	 * Returns an array of the lengths of the chromosomes of each individual in the 
	 * population from the previous generation
	 * 
	 * @return the number of codons in each individual's chromosome
	 */
