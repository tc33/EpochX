...
public class StatsEngine {
	 ...
	 * <code>null</code> if the statistic cannot be generated by this stats 
	 * engine.
	 */
	...
	private Object getGenFitnessMin() {
		...
		final double[] fitnesses = (double[]) model.getStatsManager().getGenerationStat(GEN_FITNESSES);
	...
	private Object getGenFitnessStdev() {
		...
		// Request the population fitnesses from the stats manager.
		final double[] fitnesses = (double[]) model.getStatsManager().getGenerationStat(GEN_FITNESSES);
		final double averageFitness = (Double) model.getStatsManager().getGenerationStat(GEN_FITNESS_AVE);
	...
	private Object getGenFitnessCI95() {
		...
		// Request the population fitnesses from the stats manager.
		final double[] fitnesses = (double[]) model.getStatsManager().getGenerationStat(GEN_FITNESSES);
		final double stdev = (Double) model.getStatsManager().getGenerationStat(GEN_FITNESS_STDEV);
		
		// Calculate the 95% confidence interval from the mean of the fitness values.
		if (fitnesses != null) {
	...
	private Object getGenBestProgram() {
		...
		// Request the population fitnesses from the stats manager.
		final double[] fitnesses = (double[]) model.getStatsManager().getGenerationStat(GEN_FITNESSES);
		final List<CandidateProgram> pop = (List<CandidateProgram>) model.getStatsManager().getGenerationStat(GEN_POPULATION);
