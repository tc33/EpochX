...
public class PoolSelectionManagerTest extends TestCase {
	...
	public void testPoolSizeMinusOne() {
		...
		model.getLifeCycleManager().fireConfigureEvent();
		try {
			List<CandidateProgram> pool = poolManager.getPool(pop);
	...
	public void testPoolSelectionEventsOrder() {
		...
		model.getLifeCycleManager().addPoolSelectionListener(new PoolSelectionListener() {
			@Override
	...
	public void testPoolSelectionEventRevert() {
		...
		model.getLifeCycleManager().addPoolSelectionListener(new PoolSelectionAdapter() {
			@Override
			public List<CandidateProgram> onPoolSelection(List<CandidateProgram> pool) {
				verify.append('2');
