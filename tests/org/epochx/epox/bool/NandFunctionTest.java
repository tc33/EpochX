...
/**
 * Unit tests for {@link org.epochx.epox.bool.NandFunction}
 */
public class NandFunctionTest extends NodeTestCase {

	private NandFunction node;
	...
	public void setUp() {
		...
		node = new NandFunction(child1, child2);

		super.setUp();
	}
	
	/**
	 * Tests that {@link org.epochx.epox.bool.NandFunction#evaluate()} correctly
	 * evaluates inputs.
	 */
	@Test
	public void testEvaluate() {
		...
		assertFalse("NAND of true and true should be false", node.evaluate());
		
		child1.setEvaluate(Boolean.TRUE);
		child2.setEvaluate(Boolean.FALSE);
		assertTrue("NAND of true and false should be true", node.evaluate());
		
		child1.setEvaluate(Boolean.FALSE);
		child2.setEvaluate(Boolean.TRUE);
		assertTrue("NAND of false and true should be true", node.evaluate());
		
		child1.setEvaluate(Boolean.FALSE);
		child2.setEvaluate(Boolean.FALSE);
		assertTrue("NAND of false and false should be true", node.evaluate());
	}
	
	/**
	 * Tests that {@link org.epochx.epox.bool.NandFunction#getReturnType(Class...)}
	 * returns a Boolean data-type for two Boolean inputs, and <code>null</code>
	 * for non-Boolean or incorrect number.
	 */
	@Test
	public void testGetReturnTypeNand() {
	...
	public void testEpoxParser() {
		...
		try {
			...
			Node n = parser.parse("NAND(X, X)");
			assertSame("Parsing did not return an instance of the correct node", NandFunction.class, n.getClass());
