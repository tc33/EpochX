...
public class CosecantFunctionTest extends NodeTestCase {
	...
	/**
	 * Tests that {@link org.epochx.epox.trig.CosecantFunction#getReturnType(Class...)}
	...
	public void testGetReturnTypeCsc() {
		...
		for (Class<?> type: inputTypes) {
			returnType = csc.getReturnType(type);
			assertSame("unexpected return type", Double.class, returnType);
		}
		
		returnType = csc.getReturnType(Boolean.class);
		assertNull("non-numeric type for child should be invalid", returnType);
		
		returnType = csc.getReturnType(Integer.class, Integer.class);
