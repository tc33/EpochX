...
public class PerformanceCompare {
	...
	public static void main(String[] args) throws MalformedProgramException {
		Interpreter[] interpreters = new Interpreter[] {
				...
				new ScriptingInterpreter("groovy")
		};
		
		long[] timeCounts = new long[interpreters.length];
		
		for (int i=0; i<1000*interpreters.length; i++) {
			int selected = i % interpreters.length;
			long time = test(interpreters[selected]);
	...
	private static long test(Interpreter interpreter) throws MalformedProgramException {
		...
		for (int i=0; i<1; i++) {
			...
			Object[] results = interpreter.eval(expression, args, valueSets);
