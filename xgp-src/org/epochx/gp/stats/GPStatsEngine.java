...
public class GPStatsEngine implements StatsEngine {
	...
	public Object getStat(final Object field) {
		// If not a GPStatField then we don't support it here.
		if (!(field instanceof GPStatField)) {
			return null;
		}
		...
		} else if (field == GEN_AVE_NODES_PER_DEPTH) {
		...
		} else if (field == GEN_NO_TERMINALS) {
			value = getGenNoTerminals();
		} else if (field == GEN_NO_TERMINALS_MIN) {
			value = getGenNoTerminalsMin();
		} else if (field == GEN_NO_TERMINALS_MAX) {
			value = getGenNoTerminalsMax();
		} else if (field == GEN_NO_TERMINALS_AVE) {
			value = getGenNoTerminalsAve();
		} else if (field == GEN_NO_TERMINALS_STDEV) {
			value = getGenNoTerminalsStdev();
		} else if (field == GEN_NO_DISTINCT_TERMINALS) {
			value = getGenNoDistinctTerminals();
		} else if (field == GEN_NO_DISTINCT_TERMINALS_MIN) {
			value = getGenNoDistinctTerminalsMin();
		} else if (field == GEN_NO_DISTINCT_TERMINALS_MAX) {
			value = getGenNoDistinctTerminalsMax();
		} else if (field == GEN_NO_DISTINCT_TERMINALS_AVE) {
			value = getGenNoDistinctTerminalsAve();
		} else if (field == GEN_NO_DISTINCT_TERMINALS_STDEV) {
			value = getGenNoDistinctTerminalsStdev();
		} else if (field == GEN_NO_FUNCTIONS) {
			value = getGenNoFunctions();
		} else if (field == GEN_NO_FUNCTIONS_MIN) {
			value = getGenNoFunctionsMin();
		} else if (field == GEN_NO_FUNCTIONS_MAX) {
			value = getGenNoFunctionsMax();
		} else if (field == GEN_NO_FUNCTIONS_AVE) {
			value = getGenNoFunctionsAve();
		} else if (field == GEN_NO_FUNCTIONS_STDEV) {
			value = getGenNoFunctionsStdev();
		} else if (field == GEN_NO_DISTINCT_FUNCTIONS) {
			value = getGenNoDistinctFunctions();
		} else if (field == GEN_NO_DISTINCT_FUNCTIONS_MIN) {
			value = getGenNoDistinctFunctionsMin();
		} else if (field == GEN_NO_DISTINCT_FUNCTIONS_MAX) {
			value = getGenNoDistinctFunctionsMax();
		} else if (field == GEN_NO_DISTINCT_FUNCTIONS_AVE) {
			value = getGenNoDistinctFunctionsAve();
		} else if (field == GEN_NO_DISTINCT_FUNCTIONS_STDEV) {
	...
	private Object getGenNoDistinctTerminalsMin() {
		...
		// manager.
		final int[] noTerminals = (int[]) model.getStatsManager()
				.getStat(GEN_NO_DISTINCT_TERMINALS);
	...
	private Object getGenNoDistinctFunctionsMin() {
		...
		// manager.
		final int[] noFunctions = (int[]) model.getStatsManager()
				.getStat(GEN_NO_DISTINCT_FUNCTIONS);
